...
impl JumpRateModel {
    pub fn initialize(env: Env, base: u128, multiplier: u128, jump: u128, kink: u128) {
        env.storage().persistent().set(&DataKey::BaseRatePerYear, &base);
        env.storage().persistent().set(&DataKey::MultiplierPerYear, &multiplier);
        env.storage().persistent().set(&DataKey::JumpMultiplierPerYear, &jump);
        env.storage().persistent().set(&DataKey::Kink, &kink);
        env.events().publish((Symbol::new(&env, "model_init"),), (base, multiplier, jump, kink));
    }

    pub fn get_borrow_rate(env: Env, cash: u128, borrows: u128, reserves: u128) -> u128 {
        let util = Self::utilization(cash, borrows, reserves);
        let base: u128 = env.storage().persistent().get(&DataKey::BaseRatePerYear).unwrap_or(0);
        let mult: u128 = env.storage().persistent().get(&DataKey::MultiplierPerYear).unwrap_or(0);
        let jump: u128 = env.storage().persistent().get(&DataKey::JumpMultiplierPerYear).unwrap_or(0);
        let kink: u128 = env.storage().persistent().get(&DataKey::Kink).unwrap_or(SCALE_1E6 * 8 / 10);
        if util <= kink {
    ...
    pub fn get_supply_rate(env: Env, cash: u128, borrows: u128, reserves: u128, reserve_factor: u128) -> u128 {
