...
impl SimplePeridottroller {
    pub fn initialize(env: Env, admin: Address) {
        ...
        env.storage().persistent().set(&DataKey::CloseFactorScaled, &500_000u128);
        env.storage().persistent().set(&DataKey::LiquidationIncentiveScaled, &1_080_000u128);
        // Initialize pause maps
        env.storage().persistent().set(&DataKey::PauseBorrow, &Map::<Address, bool>::new(&env));
        env.storage().persistent().set(&DataKey::PauseRedeem, &Map::<Address, bool>::new(&env));
        env.storage().persistent().set(&DataKey::PauseLiquidation, &Map::<Address, bool>::new(&env));
        env.storage().persistent().set(&DataKey::PauseDeposit, &Map::<Address, bool>::new(&env));
        env.storage().persistent().set(&DataKey::LiquidationFeeScaled, &0u128);
        env.storage().persistent().set(&DataKey::OracleMaxAgeMultiplier, &2u64);
    }
    ...
    pub fn set_close_factor(env: Env, close_factor_scaled: u128) {
        require_admin(env.clone());
        if close_factor_scaled > 1_000_000u128 { panic!("invalid close factor"); }
        env.storage().persistent().set(&DataKey::CloseFactorScaled, &close_factor_scaled);
        env.events().publish((Symbol::new(&env, "close_factor_set"),), close_factor_scaled);
    }
    ...
    pub fn set_liquidation_fee(env: Env, fee_scaled: u128) {
        require_admin(env.clone());
        if fee_scaled > 1_000_000u128 { panic!("invalid fee"); }
        env.storage().persistent().set(&DataKey::LiquidationFeeScaled, &fee_scaled);
        env.events().publish((Symbol::new(&env, "li_fee_set"),), fee_scaled);
    }

    pub fn set_oracle_max_age_multiplier(env: Env, k: u64) {
        require_admin(env.clone());
        if k == 0 { panic!("invalid max age mult"); }
        env.storage().persistent().set(&DataKey::OracleMaxAgeMultiplier, &k);
        env.events().publish((Symbol::new(&env, "oracle_k_set"),), k);
    }

    pub fn set_reserve_recipient(env: Env, recipient: Address) {
        require_admin(env.clone());
        env.storage().persistent().set(&DataKey::ReserveRecipient, &recipient);
        env.events().publish((Symbol::new(&env, "reserve_recipient_set"),), recipient);
    }

    // Rewards admin
    pub fn set_peridot_token(env: Env, token: Address) {
        require_admin(env.clone());
        env.storage().persistent().set(&DataKey::PeridotToken, &token);
        env.events().publish((Symbol::new(&env, "peridot_token_set"),), token);
    }
    ...
    pub fn set_supply_speed(env: Env, market: Address, speed_per_sec: u128) {
        ...
        let exists: Option<u128> = env.storage().persistent().get(&DataKey::SupplyIndex(market.clone()));
        if exists.is_none() {
            env.storage().persistent().set(&DataKey::SupplyIndex(market.clone()), &INDEX_SCALE_1E18);
        }
        env.storage().persistent().set(&DataKey::SupplyIndexTime(market.clone()), &now);
        env.storage().persistent().set(&DataKey::SupplySpeed(market), &speed_per_sec);
    }

    pub fn set_borrow_speed(env: Env, market: Address, speed_per_sec: u128) {
        require_admin(env.clone());
        let now = env.ledger().timestamp();
        let exists: Option<u128> = env.storage().persistent().get(&DataKey::BorrowIndex(market.clone()));
        if exists.is_none() {
            env.storage().persistent().set(&DataKey::BorrowIndex(market.clone()), &INDEX_SCALE_1E18);
        }
        env.storage().persistent().set(&DataKey::BorrowIndexTime(market.clone()), &now);
        env.storage().persistent().set(&DataKey::BorrowSpeed(market), &speed_per_sec);
    }

    pub fn set_pause_guardian(env: Env, guardian: Address) {
        require_admin(env.clone());
        env.storage().persistent().set(&DataKey::PauseGuardian, &guardian);
        env.events().publish((Symbol::new(&env, "pause_guardian_set"),), guardian);
    }

    // Pause controls
    pub fn set_pause_borrow(env: Env, market: Address, paused: bool) {
        require_admin(env.clone());
        let mut m: Map<Address, bool> = env.storage().persistent().get(&DataKey::PauseBorrow).unwrap_or(Map::new(&env));
        m.set(market.clone(), paused);
        env.storage().persistent().set(&DataKey::PauseBorrow, &m);
        env.events().publish((Symbol::new(&env, "pause_borrow"), market.clone()), paused);
    }
    pub fn is_borrow_paused(env: Env, market: Address) -> bool {
        let m: Map<Address, bool> = env.storage().persistent().get(&DataKey::PauseBorrow).unwrap_or(Map::new(&env));
        m.get(market).unwrap_or(false)
    }

    pub fn set_pause_redeem(env: Env, market: Address, paused: bool) {
        require_admin(env.clone());
        let mut m: Map<Address, bool> = env.storage().persistent().get(&DataKey::PauseRedeem).unwrap_or(Map::new(&env));
        m.set(market.clone(), paused);
        env.storage().persistent().set(&DataKey::PauseRedeem, &m);
        env.events().publish((Symbol::new(&env, "pause_redeem"), market.clone()), paused);
    }
    pub fn is_redeem_paused(env: Env, market: Address) -> bool {
        let m: Map<Address, bool> = env.storage().persistent().get(&DataKey::PauseRedeem).unwrap_or(Map::new(&env));
        m.get(market).unwrap_or(false)
    }

    pub fn set_pause_liquidation(env: Env, market: Address, paused: bool) {
        require_admin(env.clone());
        let mut m: Map<Address, bool> = env.storage().persistent().get(&DataKey::PauseLiquidation).unwrap_or(Map::new(&env));
        m.set(market.clone(), paused);
        env.storage().persistent().set(&DataKey::PauseLiquidation, &m);
        env.events().publish((Symbol::new(&env, "pause_liquidation"), market.clone()), paused);
    }

    pub fn set_pause_deposit(env: Env, market: Address, paused: bool) {
        require_admin(env.clone());
        let mut m: Map<Address, bool> = env.storage().persistent().get(&DataKey::PauseDeposit).unwrap_or(Map::new(&env));
        m.set(market.clone(), paused);
        env.storage().persistent().set(&DataKey::PauseDeposit, &m);
        env.events().publish((Symbol::new(&env, "pause_deposit"), market.clone()), paused);
    }
    ...
    pub fn pause_borrow_g(env: Env, guardian: Address, market: Address, paused: bool) {
        ...
        let Some(g) = stored else { panic!("no guardian"); };
        if g != guardian { panic!("invalid guardian"); }
        guardian.require_auth();
        let mut m: Map<Address, bool> = env.storage().persistent().get(&DataKey::PauseBorrow).unwrap_or(Map::new(&env));
        m.set(market.clone(), paused);
        env.storage().persistent().set(&DataKey::PauseBorrow, &m);
        env.events().publish((Symbol::new(&env, "pause_borrow_g"), market.clone()), paused);
    }
    ...
    pub fn pause_redeem_g(env: Env, guardian: Address, market: Address, paused: bool) {
        ...
        m.set(market.clone(), paused);
        env.storage().persistent().set(&DataKey::PauseRedeem, &m);
        env.events().publish((Symbol::new(&env, "pause_redeem_g"), market.clone()), paused);
    }
    ...
    pub fn pause_liquidation_g(env: Env, guardian: Address, market: Address, paused: bool) {
        ...
        m.set(market.clone(), paused);
        env.storage().persistent().set(&DataKey::PauseLiquidation, &m);
        env.events().publish((Symbol::new(&env, "pause_liquidation_g"), market.clone()), paused);
    }
    ...
    pub fn pause_deposit_g(env: Env, guardian: Address, market: Address, paused: bool) {
        ...
        m.set(market.clone(), paused);
        env.storage().persistent().set(&DataKey::PauseDeposit, &m);
        env.events().publish((Symbol::new(&env, "pause_deposit_g"), market.clone()), paused);
    }
    pub fn is_liquidation_paused(env: Env, market: Address) -> bool {
        let m: Map<Address, bool> = env.storage().persistent().get(&DataKey::PauseLiquidation).unwrap_or(Map::new(&env));
        m.get(market).unwrap_or(false)
    }

    pub fn is_deposit_paused(env: Env, market: Address) -> bool {
        let m: Map<Address, bool> = env.storage().persistent().get(&DataKey::PauseDeposit).unwrap_or(Map::new(&env));
        m.get(market).unwrap_or(false)
    }
    ...
    pub fn account_liquidity(env: Env, user: Address) -> (u128, u128) {
        let (_collateral_usd, borrow_usd) = Self::sum_positions_usd(env.clone(), user.clone(), None);
    ...
    pub fn preview_borrow_max(env: Env, user: Address, market: Address) -> u128 {
        ...
        if liquidity_usd == 0 { return 0u128; }
        // Price of the market underlying
        use soroban_sdk::IntoVal;
        let underlying: Address = env.invoke_contract(&market, &Symbol::new(&env, "get_underlying_token"), ().into_val(&env));
        let Some((price, scale)) = Self::get_price_usd(env.clone(), underlying.clone()) else { return 0u128; };
        ...
        let available: u128 = env.invoke_contract(&market, &Symbol::new(&env, "get_available_liquidity"), ().into_val(&env));
        if by_collateral < available { by_collateral } else { available }
    }
    ...
    pub fn preview_redeem_max(env: Env, user: Address, market: Address) -> u128 {
        ...
        // Totals in USD
        let (_collateral_usd, borrow_usd) = Self::sum_positions_usd(env.clone(), user.clone(), None);
        if borrow_usd == 0 { // no debt => can redeem all pTokens (subject to liquidity)
            let pbal: u128 = env.invoke_contract(&market, &Symbol::new(&env, "get_ptoken_balance"), (user.clone(),).into_val(&env));
            ...
            return if pbal < max_ptokens_by_liq { pbal } else { max_ptokens_by_liq };
        }
        // Collateral in USD excluding current market
        let other_collateral_usd = Self::get_collateral_excl_usd(env.clone(), user.clone(), market.clone());
        // Required local discounted collateral in USD after redeem
        let required_local_discounted_usd = if borrow_usd > other_collateral_usd { borrow_usd - other_collateral_usd } else { 0u128 };
        ...
        if cf == 0 || price == 0 { return 0u128; }
        let numerator = required_local_discounted_usd.saturating_mul(1_000_000u128).saturating_mul(scale);
        let denom = cf.saturating_mul(price);
        let min_remaining_underlying = if denom == 0 { underlying_local } else { numerator / denom };
        let max_redeem_underlying_by_health = if underlying_local > min_remaining_underlying { underlying_local - min_remaining_underlying } else { 0u128 };
        // Also limited by market available liquidity
        let available_underlying: u128 = env.invoke_contract(&market, &Symbol::new(&env, "get_available_liquidity"), ().into_val(&env));
        let clamped_underlying = if max_redeem_underlying_by_health < available_underlying { max_redeem_underlying_by_health } else { available_underlying };
    ...
    pub fn preview_repay_cap(env: Env, borrower: Address, repay_market: Address) -> u128 {
        use soroban_sdk::IntoVal;
        let close_factor: u128 = env.storage().persistent().get(&DataKey::CloseFactorScaled).unwrap_or(500_000u128);
        let debt: u128 = env.invoke_contract(&repay_market, &Symbol::new(&env, "get_user_borrow_balance"), (borrower,).into_val(&env));
        (debt.saturating_mul(close_factor)) / 1_000_000u128
    }

    // Preview pTokens to seize for a given repay_amount
    pub fn preview_seize_ptokens(env: Env, repay_market: Address, collateral_market: Address, repay_amount: u128) -> u128 {
        use soroban_sdk::IntoVal;
        // tokens
        let borrow_token: Address = env.invoke_contract(&repay_market, &Symbol::new(&env, "get_underlying_token"), ().into_val(&env));
        let coll_token: Address = env.invoke_contract(&collateral_market, &Symbol::new(&env, "get_underlying_token"), ().into_val(&env));
        // prices
        ...
        let li_scaled: u128 = env.storage().persistent().get(&DataKey::LiquidationIncentiveScaled).unwrap_or(1_080_000u128);
        ...
        let rate: u128 = env.invoke_contract(&collateral_market, &Symbol::new(&env, "get_exchange_rate"), ().into_val(&env));
    ...
    pub fn liquidate(env: Env, borrower: Address, repay_market: Address, collateral_market: Address, repay_amount: u128, liquidator: Address) {
        // top-level auth for liquidator, to allow token transfer from liquidator in nested calls
        liquidator.require_auth();
        // Check pause flags
        if Self::is_liquidation_paused(env.clone(), repay_market.clone()) || Self::is_liquidation_paused(env.clone(), collateral_market.clone()) {
        ...
        if shortfall == 0 { panic!("no shortfall"); }
        ...
        use soroban_sdk::IntoVal;
        // get borrower debt
        let debt: u128 = env.invoke_contract(&repay_market, &Symbol::new(&env, "get_user_borrow_balance"), (borrower.clone(),).into_val(&env));
        if debt == 0 { panic!("no debt"); }
        ...
        if repay == 0 { panic!("repay too small"); }

        // tokens and prices
        let borrow_token: Address = env.invoke_contract(&repay_market, &Symbol::new(&env, "get_underlying_token"), ().into_val(&env));
        let coll_token: Address = env.invoke_contract(&collateral_market, &Symbol::new(&env, "get_underlying_token"), ().into_val(&env));
        let (pb, sb) = Self::get_price_usd(env.clone(), borrow_token.clone()).expect("price borrow");
        ...
        let _: () = env.invoke_contract(&repay_market, &Symbol::new(&env, "repay_on_behalf"), (liquidator.clone(), borrower.clone(), repay).into_val(&env));
        // Route fee to reserve recipient if configured
        let liq_fee: u128 = env.storage().persistent().get(&DataKey::LiquidationFeeScaled).unwrap_or(0u128);
        if liq_fee > 0 {
            let fee_ptokens = (seize_ptokens.saturating_mul(liq_fee)) / 1_000_000u128;
            if fee_ptokens > 0 {
                if let Some(recipient) = env.storage().persistent().get::<_, Address>(&DataKey::ReserveRecipient) {
                    let _: () = env.invoke_contract(&collateral_market, &Symbol::new(&env, "seize"), (borrower.clone(), recipient, fee_ptokens).into_val(&env));
                }
            }
            let remainder = seize_ptokens.saturating_sub((seize_ptokens.saturating_mul(liq_fee)) / 1_000_000u128);
            if remainder > 0 {
                let _: () = env.invoke_contract(&collateral_market, &Symbol::new(&env, "seize"), (borrower.clone(), liquidator.clone(), remainder).into_val(&env));
            }
        } else {
            let _: () = env.invoke_contract(&collateral_market, &Symbol::new(&env, "seize"), (borrower.clone(), liquidator.clone(), seize_ptokens).into_val(&env));
        }

        env.events().publish((Symbol::new(&env, "liquidate"),), (borrower, repay_market, collateral_market, repay, seize_ptokens));
    }
    ...
    pub fn claim(env: Env, user: Address) {
        ...
        let accrued: u128 = env.storage().persistent().get(&DataKey::Accrued(user.clone())).unwrap_or(0u128);
        if accrued == 0 { return; }
        if let Some(token) = env.storage().persistent().get::<_, Address>(&DataKey::PeridotToken) {
            use soroban_sdk::IntoVal;
            let amt: i128 = if accrued > i128::MAX as u128 { i128::MAX } else { accrued as i128 };
            let _: () = env.invoke_contract(&token, &Symbol::new(&env, "mint"), (user.clone(), amt).into_val(&env));
        }
        env.storage().persistent().set(&DataKey::Accrued(user), &0u128);
    }

    pub fn get_accrued(env: Env, user: Address) -> u128 {
        env.storage().persistent().get(&DataKey::Accrued(user)).unwrap_or(0u128)
    }
    ...
    pub fn get_price_usd(env: Env, token: Address) -> Option<(u128, u128)> {
        ...
        match pd_opt {
            Some(pd) if pd.price >= 0 => {
                ...
                let k: u64 = env.storage().persistent().get(&DataKey::OracleMaxAgeMultiplier).unwrap_or(2u64);
...
impl SimplePeridottroller {
    fn accrue_market(env: Env, market: Address) {
        let now = env.ledger().timestamp();
        // supply
        let last_s: u64 = env.storage().persistent().get(&DataKey::SupplyIndexTime(market.clone())).unwrap_or(now);
        let dt_s = now.saturating_sub(last_s);
        if dt_s > 0 {
            let speed: u128 = env.storage().persistent().get(&DataKey::SupplySpeed(market.clone())).unwrap_or(0u128);
            if speed > 0 {
                use soroban_sdk::IntoVal;
                let total_ptokens: u128 = env.invoke_contract(&market, &Symbol::new(&env, "get_total_ptokens"), ().into_val(&env));
                if total_ptokens > 0 {
                    let mut idx: u128 = env.storage().persistent().get(&DataKey::SupplyIndex(market.clone())).unwrap_or(INDEX_SCALE_1E18);
                    let delta = ((speed.saturating_mul(dt_s as u128)).saturating_mul(INDEX_SCALE_1E18)) / total_ptokens;
                    idx = idx.saturating_add(delta);
                    env.storage().persistent().set(&DataKey::SupplyIndex(market.clone()), &idx);
                }
            }
            env.storage().persistent().set(&DataKey::SupplyIndexTime(market.clone()), &now);
        }
        // borrow
        let last_b: u64 = env.storage().persistent().get(&DataKey::BorrowIndexTime(market.clone())).unwrap_or(now);
        let dt_b = now.saturating_sub(last_b);
        if dt_b > 0 {
            let speed: u128 = env.storage().persistent().get(&DataKey::BorrowSpeed(market.clone())).unwrap_or(0u128);
            if speed > 0 {
                use soroban_sdk::IntoVal;
                let total_borrowed: u128 = env.invoke_contract(&market, &Symbol::new(&env, "get_total_borrowed"), ().into_val(&env));
                if total_borrowed > 0 {
                    let mut idx: u128 = env.storage().persistent().get(&DataKey::BorrowIndex(market.clone())).unwrap_or(INDEX_SCALE_1E18);
                    let delta = ((speed.saturating_mul(dt_b as u128)).saturating_mul(INDEX_SCALE_1E18)) / total_borrowed;
                    idx = idx.saturating_add(delta);
                    env.storage().persistent().set(&DataKey::BorrowIndex(market.clone()), &idx);
                }
            }
            env.storage().persistent().set(&DataKey::BorrowIndexTime(market.clone()), &now);
        }
    ...
    fn distribute_supply(env: Env, user: Address, market: Address) {
        use soroban_sdk::IntoVal;
        let idx: u128 = env.storage().persistent().get(&DataKey::SupplyIndex(market.clone())).unwrap_or(INDEX_SCALE_1E18);
        let uidx: u128 = env.storage().persistent().get(&DataKey::UserSupplyIndex(user.clone(), market.clone())).unwrap_or(INDEX_SCALE_1E18);
        if idx == uidx { return; }
        let pbal: u128 = env.invoke_contract(&market, &Symbol::new(&env, "get_ptoken_balance"), (user.clone(),).into_val(&env));
        if pbal > 0 {
            let delta_index = idx.saturating_sub(uidx);
            let add = (pbal.saturating_mul(delta_index)) / INDEX_SCALE_1E18;
            let acc: u128 = env.storage().persistent().get(&DataKey::Accrued(user.clone())).unwrap_or(0u128);
            env.storage().persistent().set(&DataKey::Accrued(user.clone()), &acc.saturating_add(add));
        }
        env.storage().persistent().set(&DataKey::UserSupplyIndex(user, market), &idx);
    }

    fn distribute_borrow(env: Env, user: Address, market: Address) {
        use soroban_sdk::IntoVal;
        let idx: u128 = env.storage().persistent().get(&DataKey::BorrowIndex(market.clone())).unwrap_or(INDEX_SCALE_1E18);
        let uidx: u128 = env.storage().persistent().get(&DataKey::UserBorrowIndex(user.clone(), market.clone())).unwrap_or(INDEX_SCALE_1E18);
        if idx == uidx { return; }
        let debt: u128 = env.invoke_contract(&market, &Symbol::new(&env, "get_user_borrow_balance"), (user.clone(),).into_val(&env));
        if debt > 0 {
            let delta_index = idx.saturating_sub(uidx);
