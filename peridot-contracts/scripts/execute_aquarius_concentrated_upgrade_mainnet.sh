#!/usr/bin/env bash
# Execute the staged Mainnet ReceiptVault/AquariusLpVault upgrades and migrate
# all three live positions from full range to actively managed narrow ranges.
# Default mode is read-only. Any mutation failure after pausing intentionally
# leaves deposits and redemptions paused for operator inspection.
set -euo pipefail
...
NEW_STRATEGY_HASH=7f1281e4fec1da073f98af6df727e5e191e10e38ac5b1dd4422b8711b93352b3
...
[[ -f "$RECEIPT_WASM" && -f "$STRATEGY_WASM" ]] || fail "pinned upgrade artifacts are missing"
...
expect "strategy artifact" "$(stellar contract info hash --wasm "$STRATEGY_WASM")" "$NEW_STRATEGY_HASH"

echo "==> Verifying binaries, bindings, supply-only policy, and timelocks"
MAX_ETA=0
SNAPSHOT_PTOKENS=()
SNAPSHOT_DEPOSITED=()
...
for i in 0 1 2; do
  ...
  [[ "$market_hash" == "$OLD_RECEIPT_HASH" || "$market_hash" == "$NEW_RECEIPT_HASH" ]] || \
    fail "$label market has unexpected hash $market_hash"
...
if [[ "$PREFLIGHT_ONLY" == "true" ]]; then
  echo "==> Read-only preflight passed; all staged timelocks are mature"
  exit 0
fi
...
echo "==> Executing ReceiptVault upgrades"
for market in "${MARKETS[@]}"; do
  if [[ "$(contract_hash "$market")" == "$OLD_RECEIPT_HASH" ]]; then
    view "$market" upgrade_wasm --new_wasm_hash "$NEW_RECEIPT_HASH" >/dev/null
    invoke "$market" upgrade_wasm --new_wasm_hash "$NEW_RECEIPT_HASH"
  fi
  expect "ReceiptVault post-upgrade hash" "$(contract_hash "$market")" "$NEW_RECEIPT_HASH"
done

echo "==> Executing strategy upgrades"
...
for i in 0 1 2; do
  ...
  if [[ "$needs" == "true" || "$needs" == '"true"' ]]; then
    # Exact live-pool migration simulation: no position is burned until this
