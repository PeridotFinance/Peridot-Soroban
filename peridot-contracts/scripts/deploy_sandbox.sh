...
soroban contract invoke $NETWORK --id "$CTRL_ID" -- \
...
TOKEN_A=$(soroban keys address bob) # placeholder asset address
TOKEN_B=$(soroban keys address carol)
...
soroban contract invoke $NETWORK --id "$VA_ID" -- set_flash_loan_fee --fee_scaled 20000
soroban contract invoke $NETWORK --id "$VB_ID" -- set_flash_loan_fee --fee_scaled 20000
