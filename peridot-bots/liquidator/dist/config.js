...
function parseMarkets(json) {
    ...
    try {
        ...
        throw new Error(`MARKETS_JSON is not valid JSON: ${error.message}`);
    }
    ...
    for (const entry of parsed) {
        ...
        if (!Number.isInteger(decimals) || decimals < 0) {
            throw new Error(`Invalid decimals for market ${entry.symbol}`);
        }
        markets.push({
            symbol: entry.symbol,
...
export function loadConfig() {
    const networkPassphrase = process.env.NETWORK_PASSPHRASE ?? 'Test SDF Future Network ; October 2022';
    ...
    try {
        ...
        throw new Error(`MIN_SHORTFALL must be an integer: ${error.message}`);
    }
    if (minShortfall < 0n) {
